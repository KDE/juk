...
{
    // handle method call org.kde.juk.collection.openFile
    QMetaObject::invokeMethod(parent(), "openFile", Q_ARG(QStringList, files));

    // Alternative:
    //static_cast<YourObjectType *>(parent())->openFile(files);
}

void CollectionAdaptor::openFile(const QString &file)
{
    // handle method call org.kde.juk.collection.openFile
    QMetaObject::invokeMethod(parent(), "openFile", Q_ARG(QString, file));

    // Alternative:
    //static_cast<YourObjectType *>(parent())->openFile(file);
}
